// Альтернативный вариант с тремя точками
export async function onRequest(context) {
  const { request, env, params } = context;
  const url = new URL(request.url);
  
  // Получаем path из params или парсим из URL
  const path = params?.path || url.pathname.replace(/^\/app\//, '').replace(/\/$/, '');
...
async function getGameData(gameId, env) {
  ...
  if (gameId === '1757350') {
    return {
      ...
      screenshots: ['/assets/images/1.png'],
